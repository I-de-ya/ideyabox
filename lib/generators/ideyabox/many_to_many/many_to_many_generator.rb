...
module Ideyabox
  module Generators
    class ManyToManyGenerator < ::Rails::Generators::Base
      ...
      #   final_string = "\n- content_for(:page_sidebar) do\n  - unless @#{child_name}.new_record?\n    = render 'admin/#{plural_resource_name}/#{plural_resource_name}'\n"

      #   inject_into_file "app/views/admin/#{plural_resource_name}/edit.html.haml", final_string, :before => "- content_for :page_header do"
      # end

      # def updating_models
      #   inject_into_file "app/models/#{child_name}.rb", "\n  has_many :#{plural_resource_name}", :after => "class #{child_name.capitalize} < ActiveRecord::Base"   
      # end
      ...
      def initialize_views_variables
        ...
        @child_contoller_class_nesting = extract_modules(child_controller_path)
      ...
      def linking_model_name
        if ActiveRecord::Base.connection.table_exists? "#{child_model}_#{plural_resource_name}"
          "#{child_model}_#{plural_resource_name}".demodulize
        elsif
          ActiveRecord::Base.connection.table_exists? "#{resource_name}_#{plural_child_model}"
          "#{child_model}_#{plural_resource_name}".demodulize
        end       
      end
      ...
      #   template "controllers/controller.rb", "app/controllers/admin/#{linking_resource_name}_controller.rb"
      # end
